...
import {
  EmailTestService,
  TestResult,
  EmailTestConfiguration,
  EmailTestData,
} from '../services/EmailTestService';
...
export function useEmailTestState(): EmailTestState & EmailTestActions {
  ...
  const getEmailTypes = useCallback(() => emailTestService.getEmailTypes(), [emailTestService]);
  ...
  const formatDisplayDate = useCallback(
    (date: Date) => emailTestService.formatDisplayDate(date),
    [emailTestService]
  );
