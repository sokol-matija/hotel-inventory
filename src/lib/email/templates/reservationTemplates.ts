/**
 * HTML template generators for reservation-related emails.
 * Pure functions — no network calls, no side effects.
 * Supports languages: en (English), de (German), it (Italian).
 */
...
import { getEmailStyles } from '@/lib/email/emailStyles';
...
export interface HotelInfoEmailData {
  guest: Guest;
...
export function generateWelcomeEmail(
  ...
  const guestCount = reservation.number_of_guests ?? reservation.adults;
            ...
            <div class="section booking-details">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">${guestCount} guest${guestCount > 1 ? 's' : ''}</div>
                    </div>
                    <div class="info-item">
 ...
 */
export function generateThankYouEmail(data: HotelInfoEmailData): EmailTemplate {
  const { guest, reservation, room } = data;
...
/**
 * Generate summer season reminder email (marketing / re-engagement).
 */
export function generateSeasonReminderEmail(data: { guest: Guest }): EmailTemplate {
  const { guest } = data;
...
/**
 * Generate stay-reminder email for an upcoming reservation.
 */
export function generateReminderEmail(data: HotelInfoEmailData): EmailTemplate {
  const { guest, reservation, room } = data;
  const checkInDate = format(new Date(reservation.check_in_date), 'EEEE, MMMM do, yyyy');
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <strong>Guests:</strong> ${guestCount} guest${guestCount > 1 ? 's' : ''}</p>
            </div>
...
/**
 * Dispatch helper: generate the right template by EmailType.
 */
export function generateEmailByType(
  ...
  switch (type) {
    case 'welcome':
      return generateWelcomeEmail(data as HotelInfoEmailData, language);
    case 'thankyou':
      return generateThankYouEmail(data as HotelInfoEmailData);
    case 'reminder':
      return generateSeasonReminderEmail(data as { guest: Guest });
    default:
