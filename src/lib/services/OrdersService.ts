  ...
  private async deductInventoryFIFO(orderItems: OrderItem[]): Promise<void> {
    for (const orderItem of orderItems) {
      ...
      if (remainingQuantity > 0) {
        throw new Error(
          `Insufficient stock for ${orderItem.itemName}. Missing ${remainingQuantity} ${orderItem.unit}.`
        );
      }
    }
  }
    ...
    try {
      ...
      const orderData = {
        order: {
          ...
          paymentMethod: (paymentMethod === 'cash' ? 'immediate_cash' : 'immediate_card') as
            | 'immediate_cash'
            | 'immediate_card',
        ...
        hotelInfo: {
          ...
          oib: '87246357068',
          fiscalNumber: `HP-${new Date().getFullYear()}-${Math.floor(Math.random() * 999999)
            .toString()
            .padStart(6, '0')}`,
        },
        timestamp: new Date(),
  ...
  validateOrder(orderItems: OrderItem[]): {
    ...
    orderItems.forEach((item) => {
      ...
      if (item.quantity > item.availableStock) {
        errors.push(
          `${item.itemName}: Requested ${item.quantity} but only ${item.availableStock} available`
        );
      }
    });
