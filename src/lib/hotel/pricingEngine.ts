...
export class HotelPricingEngine {
  ...
  public calculatePricing(input: PricingCalculationInput): DetailedPricingCalculation {
    ...
    // Get base room rate
    const baseRoomRate = this.getBaseRoomRate(
      input.roomType,
      seasonalPeriod,
      input.pricingTierMultiplier
    );
  ...
  private calculateDiscounts(
    ...
    const accommodationBeforeVip =
      baseRoomRate * nights * (children.length + 1) -
    ...
    const totalDiscounts =
      children0to3.amount + children3to7.amount + children7to14.amount + vipDiscount;
  ...
  private calculateServices(
    ...
    const parkingNights = isRoom401
      ? Math.max(0, nights - PRICING_RULES_2026.room401Rules.includedParkingSpaces)
      : needsParking
        ? nights
        : 0;
  ...
  private calculateTourismTaxTotal(
    adults: number,
    children: GuestChild[],
    nights: number,
  ...
  private calculateVAT(
    ...
    services: ReturnType<HotelPricingEngine['calculateServices']>
  ) {
    // Accommodation VAT (13% already included in prices)
    const accommodationVAT =
      accommodationTotal *
      (PRICING_RULES_2026.vatRates.accommodation / (1 + PRICING_RULES_2026.vatRates.accommodation));

    // Services VAT (25% on parking, pets, towels)
    const servicesVAT =
      services.parking.vatAmount + services.pets.vatAmount + services.towelRental.vatAmount;
  ...
  // Validation methods
  public validateRoom401Booking(
    checkIn: Date,
    checkOut: Date,
    _existingReservations: unknown[]
    ...
    if (nights < PRICING_RULES_2026.room401Rules.minimumStay) {
      errors.push(
        `Room 401 requires minimum ${PRICING_RULES_2026.room401Rules.minimumStay} night stay. Selected: ${nights} nights.`
      );
    }
