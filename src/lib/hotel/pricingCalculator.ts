...
import { SeasonalPeriod, PricingCalculation, GuestChild, Room, HOTEL_CONSTANTS } from './types';
...
export function calculatePricing(
  ...
  const shortStayFee =
    numberOfNights < HOTEL_CONSTANTS.MIN_NIGHTS_NO_SUPPLEMENT
      ? subtotal * HOTEL_CONSTANTS.SHORT_STAY_SUPPLEMENT
      : 0;
  ...
  const vatAmount =
    roomSubtotalAfterDiscounts * (HOTEL_CONSTANTS.VAT_RATE / (1 + HOTEL_CONSTANTS.VAT_RATE));
...
export function formatPricingBreakdown(pricing: PricingCalculation): {
  ...
  if (pricing.discounts.children3to7 > 0) {
    details.push({
      label: 'Children 3-7 discount (50%)',
      amount: -pricing.discounts.children3to7,
      isDiscount: true,
    });
  }
  if (pricing.discounts.children7to14 > 0) {
    details.push({
      label: 'Children 7-14 discount (20%)',
      amount: -pricing.discounts.children7to14,
      isDiscount: true,
    });
  }
...
export function getQuickPricing(
  roomType: string,
