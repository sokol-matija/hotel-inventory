...
describe('Populate Test Data', () => {
  test('should populate guests table with test data', async () => {
    try {
      ...
      const { data, error } = await supabase.from('guests').insert(testGuests).select();
      ...
      data?.forEach((guest, index) => {
        console.log(
          `   ${index + 1}. ${guest.first_name} ${guest.last_name} (${guest.nationality})`
        );
      });
    } catch (err) {
  ...
  test('should populate rooms table with test data (without room_types)', async () => {
    try {
      ...
      const { data, error } = await supabase.from('rooms').insert(testRooms).select();
  ...
  test('should create test reservations with existing guests and rooms', async () => {
    try {
      ...
      const testReservations = [
        {
          ...
          check_out: new Date(today.getTime() + 3 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split('T')[0], // +3 days
          adults: 2,
          children: 0,
          ...
          seasonal_period: 'A',
          base_room_rate: 150.0,
          number_of_nights: 2,
          subtotal_accommodation: 300.0,
          vat_accommodation: 75.0,
          total_amount: 375.0,
          total_vat_amount: 75.0,
        ...
        },
        {
          ...
          check_out: new Date(today.getTime() + 8 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split('T')[0], // +8 days
          adults: 1,
          children: 0,
          ...
          seasonal_period: 'B',
          base_room_rate: 180.0,
          number_of_nights: 3,
          subtotal_accommodation: 540.0,
          vat_accommodation: 135.0,
          total_amount: 675.0,
          total_vat_amount: 135.0,
          confirmation_number: 'TEST-002',
          payment_status: 'paid',
        },
      ];

      const { data, error } = await supabase.from('reservations').insert(testReservations).select();
      ...
      data?.forEach((reservation, index) => {
        console.log(
          `   ${index + 1}. ${reservation.confirmation_number} - Room ${rooms.find((r) => r.id === reservation.room_id)?.number} (${reservation.status})`
        );
      });
    } catch (err) {
