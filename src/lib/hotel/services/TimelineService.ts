...
export class TimelineService {
  ...
  public getRoomAvailability(
    ...
    while (currentDate < endDate) {
      ...
      results.push({
        ...
        conflictLevel:
          dayReservations.length > 1 ? 'full' : dayReservations.length === 1 ? 'partial' : 'none',
      });
  ...
  public getRoomOccupancyStats(
    ...
    const availableRooms = HOTEL_POREC_ROOMS.filter((room) => !occupiedRoomIds.has(room.id));
   ...
   */
  public navigateTimeline(currentDate: Date, action: 'PREV' | 'NEXT' | 'TODAY'): Date {
    switch (action) {
      case 'PREV':
