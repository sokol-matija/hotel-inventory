...
function calculate2026Pricing(
  ...
  try {
    ...
    return {
      ...
      totalFees:
        result.services.tourism.total +
        result.totalVATAmount +
        result.services.pets.total +
        result.services.parking.total,
...
export const SAMPLE_GUESTS: Guest[] = [
  ...
  },
  {
    ...
    isVip: false,
    vipLevel: 1,
    children: [createChild('Max Schmidt', 8), createChild('Emma Schmidt', 5)],
    totalStays: 1,
  ...
  // Italian tourists
  {
    ...
    isVip: false,
    vipLevel: 1,
    children: [createChild('Sofia Rossi', 12)],
    totalStays: 2,
  ...
  // Austrian tourists
  {
    ...
    isVip: true,
    vipLevel: 3,
    children: [createChild('Lukas Huber', 15), createChild('Anna Huber', 3)],
    totalStays: 4,
  ...
  // Croatian domestic tourists
  {
    ...
    isVip: false,
    vipLevel: 1,
    children: [createChild('Petra Horvat', 10)],
    totalStays: 2,
  ...
  // Slovenian tourists (neighboring country)
  {
    ...
    isVip: true,
    vipLevel: 2,
    children: [createChild('Nik Novak', 7), createChild('Lara Novak', 4)],
    totalStays: 3,
  ...
  // Dutch tourists
  {
    ...
    isVip: false,
    vipLevel: 1,
    children: [createChild('Emma van der Berg', 13), createChild('Tim van der Berg', 9)],
    totalStays: 2,
...
export function generateDemoReservations(): Reservation[] {
  ...
  for (let weekOffset = 0; weekOffset < 3; weekOffset++) {
    ...
    for (let i = 0; i < roomsToFill; i++) {
      ...
      const childrenCount =
        remainingCapacity > 0 && Math.random() < 0.3
  ...
  showcaseReservations.forEach((showcase) => {
    ...
    const index = reservations.findIndex(
      (r) =>
        ...
        r.checkIn < showcase.checkOut &&
        r.checkOut > showcase.checkIn
    );
    if (index >= 0) {
...
export function getFutureReservations(): Reservation[] {
  return SAMPLE_RESERVATIONS.filter(
    (reservation) => reservation.checkIn > new Date() && reservation.status === 'confirmed'
  );
}
...
export const SAMPLE_DATA_STATS = {
  ...
  nationalityBreakdown: SAMPLE_GUESTS.reduce(
    (acc, guest) => {
  ...
  statusBreakdown: SAMPLE_RESERVATIONS.reduce(
    (acc, reservation) => {
