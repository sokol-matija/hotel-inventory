...
export default function EditCompanyModal({ isOpen, onClose, company }: EditCompanyModalProps) {
  const { updateCompany, validateOIB } = useHotel();
  ...
  useEffect(() => {
    if (company) {
      setFormData({
        ...
        country: convertToDisplayName(company.address.country), // Convert ISO code to display name
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">Update {company.name} details</p>
            </div>
          </div>
