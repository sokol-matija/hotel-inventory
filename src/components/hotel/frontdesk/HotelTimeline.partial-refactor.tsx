...
import { useTimelineState } from '../../../lib/hotel/hooks/useTimelineState';
import { TimelineService } from '../../../lib/hotel/services/TimelineService';
...
// Main timeline component - Refactored to use services and hooks
export default function HotelTimeline({ isFullscreen = false, onToggleFullscreen }: HotelTimelineProps) {
  ...
  // Use our new timeline state hook - replaces all the individual useState calls
  const {
    state,
    timelineDates,
    roomsGroupedByFloor,
    ...
    toggleMoveMode,
    updateState
  } = useTimelineState(reservations);
  
  // Context menu positioning - now using service
  const calculateContextMenuPosition = (e: React.MouseEvent, menuWidth = 180, menuHeight = 300) => {
    return timelineService.calculateContextMenuPosition(e.clientX, e.clientY, menuWidth, menuHeight);
  };

  // Escape key listener to cancel drag-to-create
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && state.isDragCreating) {
        resetDragCreate();
      }
    };
  ...
  }, [state.isDragCreating, resetDragCreate]);
  
  // Current occupancy data for the overview date - simplified using timeline service
  const currentOccupancy = useMemo(() => {
    const targetDate = startOfDay(state.overviewDate);
    ...
    return occupancy;
  }, [reservations, state.overviewDate]);

  // Overview date navigation - simplified
  const handleOverviewNavigate = (action: 'PREV' | 'NEXT' | 'TODAY') => {
    let newDate: Date;
    switch (action) {
      case 'PREV':
        newDate = addDays(state.overviewDate, -1);
        break;
      case 'NEXT':
        newDate = addDays(state.overviewDate, 1);
        break;
    ...
    updateState({ overviewDate: newDate });
  };
  
  // Event handlers - now using timeline state management
  ...
  const handleConfirmRoomChange = async () => {
    if (!state.roomChangeDialog.reservation || !state.roomChangeDialog.targetRoom || !state.roomChangeDialog.currentRoom) return;
    ...
    try {
      ...
      const pricingInput = {
        roomType: state.roomChangeDialog.targetRoom.type,
        roomId: state.roomChangeDialog.targetRoom.id,
        checkIn: state.roomChangeDialog.newCheckIn!,
        checkOut: state.roomChangeDialog.newCheckOut!,
        adults: state.roomChangeDialog.reservation.adults,
        children: state.roomChangeDialog.reservation.children,
        hasPets: state.roomChangeDialog.reservation.petFee > 0,
        needsParking: state.roomChangeDialog.reservation.parkingFee > 0,
        pricingTierId: '2026-standard',
        isRoom401: state.roomChangeDialog.targetRoom.id === '401'
      };
      ...
      await updateReservation(state.roomChangeDialog.reservationId, updatedReservationData);

      const successMessage = `${state.roomChangeDialog.guest?.name || 'Guest'} moved from ${formatRoomNumber(state.roomChangeDialog.currentRoom)} to ${formatRoomNumber(state.roomChangeDialog.targetRoom)} with updated pricing`;
