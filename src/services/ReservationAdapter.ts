...
export class ReservationAdapter {
  ...
  private async getReservationLegacySchema(
    reservationId: string
  ): Promise<NormalizedReservationData> {
    ...
    if (additionalAdultsMatch) {
      const additionalIds = additionalAdultsMatch[1]
        .split(',')
  ...
  private async createReservationNewSchema(
    ...
    for (let i = 0; i < reservationData.guests.length; i++) {
      ...
      // Set primary guest
      if (i === 0) {
        await supabase.from('reservations').update({ guest_id: guest.id }).eq('id', reservation.id);
      }
  ...
  private async createReservationLegacySchema(
    ...
    const notes =
      additionalGuestIds.length > 0 ? `additional_adults:${additionalGuestIds.join(',')}` : '';
