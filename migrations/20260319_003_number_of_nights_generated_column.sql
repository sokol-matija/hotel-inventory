-- Migration: Make reservations.number_of_nights a generated column
-- Why: number_of_nights is purely derived from check_out_date - check_in_date.
--      Storing it as a plain integer requires callers to keep it in sync manually.
--      A generated column eliminates that burden and prevents drift.
-- Note: Two dependent views (reservation_with_all_guests, reservations_with_enums) were
--       dropped via CASCADE and recreated with the new generated column.

-- Step 3a: Drop old plain column
ALTER TABLE reservations DROP COLUMN IF EXISTS number_of_nights CASCADE;

-- Step 3b: Re-add as a generated column
ALTER TABLE reservations
  ADD COLUMN number_of_nights INTEGER GENERATED ALWAYS AS
    ((check_out_date - check_in_date)) STORED;

-- Recreate reservation_with_all_guests (references number_of_nights)
...
-- Recreate reservations_with_enums (references number_of_nights)
